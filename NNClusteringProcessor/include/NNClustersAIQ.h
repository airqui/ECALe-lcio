/*_* -*- Mode: C++; tab-width: 2; indent-tabs-mode: nil; c-basic-offset: 2 -*- */
#ifndef NNClustersAIQ_h
#define NNClustersAIQ_h 1

/** File with various classes for a generic nearest neighbour type clustering.
 *
 *  @author F.Gaede (DESY)
 *  @version $Id: NNClustersAIQ.h,v 1.5 2007-06-05 15:35:49 engels Exp $
 */
...
public:
  ...
  NNDistance(float dCut, float eCut) : _dCutSquared( dCut*dCut ) , _dCut(dCut), _eCut(eCut)  {} 
  ...
  inline bool mergeHits( GenericHit<HitClass>* h0, GenericHit<HitClass>* h1){
    ...
    double e0 =  h0->first->getEnergy() ;
    double e1 =  h1->first->getEnergy() ;
    double mod0=sqrt(pos0[0]*pos0[0] + pos0[1]*pos0[1] + pos0[2]*pos0[2] );
    double mod1=sqrt(pos1[0]*pos1[0] + pos1[1]*pos1[1] + pos1[2]*pos1[2] );
    double cos= pos0[0]+pos1[0]+pos0[1]+pos1[1]+pos0[2]+pos1[2];
    cos/=(mod0*mod1);

    //if(2*e0*e1*(1-cos)< _dCut) return true;
    /*if( fabs(pos0[2]-pos1[2])<0.1 && (( pos0[0] - pos1[0] ) * ( pos0[0] - pos1[0] ) +
      ( pos0[1] - pos1[1] ) * ( pos0[1] - pos1[1] )  < 11*11) ) return true;
